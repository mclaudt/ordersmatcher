...
class StockSessionSpec extends FlatSpec {
  ...
  "A basic two-client session " should "produce a simple exchange" in {
    ...
    val initialState: Iterator[ClientState] = List(
      ClientState(client1, 100, scala.collection.mutable.Map.empty),
      ClientState(client2, 0, scala.collection.mutable.Map('A' -> 1))
    ).toIterator
  ...
  "Session " should "prevent orders of client with himself" in {
    ...
    val initialState: Iterator[ClientState] = List(
      ClientState(client1, 100, scala.collection.mutable.Map.empty),
      ClientState(client2, 100, scala.collection.mutable.Map('A' -> 1))
    ).toIterator
  ...
  "Session " should "prevent orders which lead to negative account value" in {
    ...
    val initialState: Iterator[ClientState] = List(
      ClientState(client1, 100, scala.collection.mutable.Map.empty),
      ClientState(client2, 0, scala.collection.mutable.Map('A' -> 10))
    ).toIterator
